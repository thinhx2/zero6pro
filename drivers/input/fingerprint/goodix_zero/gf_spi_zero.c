...
#define GF_SPIDEV_NAME     "goodix,fingerprintzero6"
			...
			break;
                ...
                for(i = 0; i< ARRAY_SIZE(key_map); i++) {
                    if(key_map[i].val == gf_key.key){
						...
						if(116 == gf_key.key ){
							input_report_key(gf_dev->input, KEY_FP_TAP, gf_key.value);
							input_sync(gf_dev->input);
							pr_warn("key %d %d sent \n", KEY_FP_TAP,gf_key.value);
							if ((gf_dev->wait_finger_down == true) && (gf_dev->device_available == 1) && (gf_dev->fb_black == 1)) {
									gf_dev->wait_finger_down = false;
									printk(" gf_irq wait_finger_down\n");
                ...
                if(i == ARRAY_SIZE(key_map)) {
                    pr_warn("key %d %dnot support yet \n", gf_key.key,gf_key.value);
                    retval = -EFAULT;
                }

...
#define NVT_GESTURE_EN "nvt_gesture"
...
static struct proc_dir_entry *NVT_proc_gesture_enable_entry;
...
static ssize_t nvt_gesture_enable_write(struct file *filp, const char __user *buf,
        size_t count, loff_t *f_pos)
{
	...
	ts->nvt_gesture_en = input > 0 ? GESTURE_ENABLE : GESTURE_DISABLE;

	NVT_LOG("Gesture %s\n", ts->nvt_gesture_en ? "enable" : "disable");
...
static ssize_t nvt_gesture_enable_read(struct file *file, char __user *buf,size_t count, loff_t *ppos)
{
	int ret = 0;
	int len;
	uint8_t *ptr = NULL;
...
static const struct file_operations nvt_gesture_enable_fops = {
	.owner = THIS_MODULE,
	.write = nvt_gesture_enable_write,
	.read = nvt_gesture_enable_read,
	.llseek = no_llseek,
};
...
int32_t nvt_extra_proc_init(void)
{
	...
	NVT_proc_gesture_enable_entry = proc_create(NVT_GESTURE_EN, 0222, NULL,&nvt_gesture_enable_fops);
	if (NVT_proc_gesture_enable_entry == NULL) {

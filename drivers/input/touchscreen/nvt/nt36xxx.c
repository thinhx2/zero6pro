...
void nvt_bootloader_reset(void)
{
	...
	//msleep(35);
	NVT_LOG("nvt_bootloader_reset finished !\n");
}
...
info_retry:
	...
	ts->fw_ver = buf[1];
	NVT_ERR("KENIO DEBUG FW VERSION IS %d\n", ts->fw_ver);
	ts->x_num = buf[3];
	ts->y_num = buf[4];
...
static int32_t nvt_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	init_tp_fm_info(0, fw_version, "Novatek");
#endif
...
#if WAKEUP_GESTURE
		/* default enable gesture feature */
		ts->nvt_gesture_en = true;

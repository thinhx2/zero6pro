...
{
	if (fw_entry) {
		NVT_LOG("update_firmware_release start\n");
		release_firmware(fw_entry);
		NVT_LOG("update_firmware_release finished\n");
	}
	fw_entry=NULL;
}
...
void Boot_Update_Firmware(struct work_struct *work)
{
	...
	if (ret < 0) {	// read firmware checksum failed
		...
		ret = nvt_check_fw_reset_state(RESET_STATE_INIT);
	  	NVT_LOG("nvt_check_fw_reset_state value %d\n",ret);	
		if (ret) {
			NVT_LOG("check fw reset state failed\n");
			Update_Firmware();
			NVT_LOG("update_firmware finished\n");	
		}
	}
	...
	memset(fw_version, 0, sizeof(fw_version));
 	NVT_ERR("kenio debug 1 nvt_fw_ver is %d\n",ts->fw_ver);	
	sprintf(fw_version, "[FW]0x%x, [IC]NT36672A", ts->fw_ver);
	update_tp_fm_info(fw_version);
 	NVT_ERR("kenio debug 2 fw_version is %s\n",fw_version);	
#endif

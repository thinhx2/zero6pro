...
static void tfa98xx_monitor(struct work_struct *work)
{
	...
	if (!tfa98xx->input) {
		 ...
		 */
		val = snd_soc_read(tfa98xx->codec, TFA98XX_STATUSREG);
		val1 = snd_soc_read(tfa98xx->codec, TFA98XX_SYS_CTRL);
		pr_debug("SYS_STATUS0: 0x%04x\n", val);
		pr_debug("SYS_CTRL09: 0x%04x\n", val1);
		if ((TFA98XX_STATUSREG_ACS & val) ||
		    (TFA98XX_STATUSREG_WDS & val) ||
		    (!(TFA98XX_STATUSREG_AMPS & val)) ||
		    (!(TFA98XX_STATUSREG_SWS & val)) ||
		    (!(TFA98XX_SYS_CTRL_SBSL & val1))) {
			...
			if (TFA98XX_STATUSREG_WDS & val)
				pr_err("ERROR: WDS\n");
			if (!(TFA98XX_STATUSREG_AMPS & val))
				pr_err("ERROR: AMPS\n");
			if (!(TFA98XX_STATUSREG_SWS & val))
				pr_err("ERROR: SWS\n");
			if (!(TFA98XX_SYS_CTRL_SBSL & val1))
				pr_err("ERROR: SBSL\n");
